...
export const SeasonGames = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await loadData(`/games?season=${season}`);
                const gamesWithOpenState = data.response.map(game => ({
                    ...game,
                    isOpen: false
                }));
    ...
    const handleSubmit = (e) => {
        ...
        e.preventDefault();
        games.forEach(item => {
            if (item.teams.visitors.nickname && (
                    item.teams.visitors.nickname.toLowerCase().includes(searchLowerCase) || 
                    item.teams.home.nickname.toLowerCase().includes(searchLowerCase)
                )
            ) {
                results.push(item);
            } else if(item.date.start && verifyString(search, item.date.start)){
                results.push(item);
            }
        });
        console.log(results);
    ...
    const renderGames = (gamesToRender) => {
        return gamesToRender.map((game, index) => (
            <div key={game.id}>
                <button className={styles.primaryBtn} onClick={() => handleOpenModal(index)}>
                    {convertDate(game.date.start)} / {game.teams.home.nickname} X {game.teams.visitors.nickname}
                </button>
                {game.isOpen && (
                    <ModalWrapper
    ...
    const currentGames = search !== '' ? result.slice(indexOfFirstGame, indexOfLastGame) : games.slice(indexOfFirstGame, indexOfLastGame);

    useEffect(() => {
        setMaxPages(Math.round(currentGames.length/pageSize))
    }, [currentGames.length])
                ...
                <div className={styles.gamesBoard}>
                    <form onSubmit={(e) => handleSubmit(e)}>
                        <SearchBar
                            ...
                            value={search}
                        />
                        <Button type="submit" text="Pesquisar jogos">Pesquisar</Button>
                    </form>

                    {search !== '' ? renderGames(result) : renderGames(currentGames)}
